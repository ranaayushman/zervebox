    ...
    <section className='h-full w-full'>
      <MaxWidthWrapper className='h-full'>
        ...
        <div className='grid grid-cols-1'>
          <div>
            <h2 className='text-center text-3xl md:text-5xl font-semibold'>Why <span className='text-[#F5753A]'>Zerve</span><span className='text-[#3E479B]'>box</span> ?</h2>
          </div>
          <div className='flex flex-col md:flex-row justify-between py-6 md:py-10'>
            {FAQ.map((item, index) => (
              <div key={index} className='p-4 flex-1'>
                <h3 className='text-xl md:text-2xl font-bold mb-3'>{item.heading}</h3>
